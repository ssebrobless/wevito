#!/usr/bin/env python3
"""Clean squirrel runtime rows by reseeding from the best local idle silhouettes.

The squirrel rows are not corrupt enough for transparent-noise cleanup, but many
action rows contain ghosted duplicate poses and weak motion. This tool keeps the
existing squirrel art style and redraws only by moving each row's own idle frames
within its transparent canvas.
"""
...
def main() -> int:
    ...
    squirrel_root = args.runtime_root / "squirrel"
    rows: list[RowRecord] = []
    families = [family for family in args.families if family in MOTION_PATTERNS]
    for row_dir in sorted(squirrel_root.glob("*/*/*")):
    ...
    (args.output_root / "squirrel-motion-cleanup.json").write_text(json.dumps(payload, indent=2), encoding="utf-8")
    write_markdown(args.output_root / "squirrel-motion-cleanup.md", payload)
