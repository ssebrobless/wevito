...
def frame_paths_for(directory: Path) -> list[Path]:
    frames: list[Path] = []
    for animation_name, count in ANIMATION_ORDER:
        for index in range(count):
            frames.append(directory / f"{animation_name}_{index:02d}.png")
    return frames


def render_species_sheet(sprite_root: Path, output_root: Path, species: str) -> Path:
    ...
    for row_index, (age_stage, gender) in enumerate(ROW_ORDER):
        ...
        for frame_index, frame_path in enumerate(frame_paths_for(directory)):
...
def render_color_sheet(sprite_root: Path, output_root: Path, species: str) -> Path:
    ...
    for row_index, (age_stage, gender) in enumerate(ROW_ORDER):
        ...
        for color_index, color in enumerate(COLORS):
            ...
            frame_path = directory / "idle_00.png"
            frame = Image.open(frame_path).convert("RGBA")
            frame = frame.resize(FRAME_SIZE, Image.Resampling.NEAREST)
...
def main() -> None:
    ...
    for species in species_ids:
        outputs.append(render_species_sheet(args.sprite_root, args.output_root, species))
        outputs.append(render_color_sheet(args.sprite_root, args.output_root, species))
