#!/usr/bin/env python3
"""Clean goose runtime rows by rebuilding motion from local idle silhouettes.

Several goose runtime action rows contain duplicated bodies, puddle/base
artifacts, and pose jumps that read as transformation instead of animation. This
tool keeps the current goose palette and silhouette source by using each row's
own idle frames as seeds, then applies small in-canvas offsets per family.
"""
...
def main() -> int:
    ...
    (args.output_root / "goose-motion-cleanup.json").write_text(json.dumps(payload, indent=2), encoding="utf-8")
    write_markdown(args.output_root / "goose-motion-cleanup.md", payload)
