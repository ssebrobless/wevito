#!/usr/bin/env python3
"""Clean deer runtime rows by rebuilding motion from local idle silhouettes.

Many deer rows contain duplicate bodies, scale jumps, or mismatched action
sprites. This tool preserves each row's local species/age/gender/color identity
by using its own idle frames as the source for conservative runtime motion.
"""
...
def choose_safe_idle_source(idle_paths: list[Path]) -> Path:
    ...
    for idle_path in idle_paths:
        ...
        if bbox is None:
            continue
        top_margin = bbox[1]
        right_margin = source.width - bbox[2]
        bottom_margin = source.height - bbox[3]
        left_margin = bbox[0]
        score = min(top_margin, right_margin, bottom_margin, left_margin)
        if score > best_score:
            best_path = idle_path
            best_score = score
    return best_path
...
def main() -> int:
    ...
    (args.output_root / "deer-motion-cleanup.json").write_text(json.dumps(payload, indent=2), encoding="utf-8")
    write_markdown(args.output_root / "deer-motion-cleanup.md", payload)
