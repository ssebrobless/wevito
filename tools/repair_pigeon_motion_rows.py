#!/usr/bin/env python3
"""Clean pigeon runtime rows by rebuilding motion from safe local silhouettes.

The pigeon runtime sheets still show major row-to-row style and scale drift in
game, especially around baby and adult action rows. This tool keeps each
age/gender/color folder local by selecting the safest compact idle frame for
that exact row, then rebuilding the runtime motion families with conservative
offsets inside the existing canvas.
"""
...
def idle_score(path: Path) -> tuple[int, int, int]:
    ...
    # Prefer safe margins, then compact readable body shapes. That avoids
    # turning a huge wing-flare or crop-touching frame into every action row.
...
def main() -> int:
    ...
    pigeon_root = args.runtime_root / "pigeon"
    rows: list[RowRecord] = []
    families = [family for family in args.families if family in MOTION_PATTERNS]
    for row_dir in sorted(pigeon_root.glob("*/*/*")):
    ...
    (args.output_root / "pigeon-motion-cleanup.json").write_text(json.dumps(payload, indent=2), encoding="utf-8")
    write_markdown(args.output_root / "pigeon-motion-cleanup.md", payload)
