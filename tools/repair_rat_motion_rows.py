#!/usr/bin/env python3
"""Clean rat runtime rows by reseeding from each row's local idle silhouettes.

The rat runtime rows are structurally valid, but several action rows read as
muddy duplicate/afterimage stacks in-game. This conservative repair does not
generate or repaint animal pixels. It crops each row's own idle silhouettes,
re-centers them with a stable bottom anchor, and applies small whole-sprite
motion offsets so the rows stay readable in the overlay.
"""
...
def write_markdown(path: Path, payload: dict) -> None:
    lines = [
        ...
        "## Method",
        "",
        "Rebuilds rat runtime rows from each row's local idle silhouettes with stable bottom anchoring.",
        "No generation, repainting, recoloring, scaling, mirroring, source-board edits, or prop-anchor edits.",
        "",
        "## Rows",
    ]
...
def main() -> int:
    ...
    (args.output_root / "rat-motion-cleanup.json").write_text(json.dumps(payload, indent=2), encoding="utf-8")
    write_markdown(args.output_root / "rat-motion-cleanup.md", payload)
