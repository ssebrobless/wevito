...
public sealed class ProposalQualityMetricsServiceObserverWiringTests
{
    private const string ObserverFlagName = "snapshot_v0_invariant_observer_in_proposal_quality_metrics_enabled";
    ...
    [Fact]
    public void ProductTruth_proposal_quality_metrics_observer_flag_registered_default_false()
    {
    ...
    [Fact]
    public void ProductTruth_proposal_quality_metrics_service_ctor_backward_compatible()
    {
        var fixture = ObserverFixture.Create(observerEnabled: false);

        _ = new ProposalQualityMetricsService(fixture.DatabasePath, fixture.ArtifactRoot);
        _ = new ProposalQualityMetricsService(fixture.DatabasePath, fixture.ArtifactRoot, fixture.KillSwitch);

        var parameters = typeof(ProposalQualityMetricsService)
            .GetConstructors()
            .SelectMany(ctor => ctor.GetParameters())
            .ToArray();
        var watchdogParameter = parameters.Single(parameter => parameter.Name == "watchdog");
        var settingsProviderParameter = parameters.Single(parameter => parameter.Name == "settingsProvider");
        ...
        Assert.Equal(typeof(Func<IReadOnlyDictionary<string, string>>), settingsProviderParameter.ParameterType);
        Assert.True(settingsProviderParameter.HasDefaultValue);
        Assert.Null(settingsProviderParameter.DefaultValue);
    }

    [Fact]
    public void ProductTruth_proposal_quality_metrics_snapshot_flag_off_does_not_invoke_watchdog()
    {
        var fixture = ObserverFixture.Create(observerEnabled: false);
        ...
        _ = service.Snapshot(fixture.OperationId, Now);

        Assert.Empty(fixture.InvariantRows());
    }

    [Fact]
    public void ProductTruth_proposal_quality_metrics_snapshot_flag_on_records_exactly_one_invariant_row()
    {
    ...
    [Fact]
    public void ProductTruth_proposal_quality_metrics_snapshot_flag_on_null_watchdog_does_not_throw()
    {
        var fixture = ObserverFixture.Create(observerEnabled: true);
        fixture.Record(SelfImprovementPacketKinds.ApplyV0Completed, fixture.OperationId, Now.AddMinutes(-1));
        var service = fixture.CreateService(withWatchdog: false);

        var snapshot = service.Snapshot(fixture.OperationId, Now);
    ...
    [Fact]
    public void ProductTruth_proposal_quality_metrics_service_source_does_not_call_scan_or_emit_directly()
    {
    ...
    [Fact]
    public void ProductTruth_proposal_quality_metrics_no_new_audit_packet_kind()
    {
        Assert.Equal(KnownPacketKindCount, PlainLanguageExplainer.KnownPacketKinds.Count);
    }

    [Fact]
    public void ProductTruth_proposal_quality_metrics_service_source_references_killswitch()
    {
        var source = File.ReadAllText(ServiceSourcePath());
    ...
    private sealed class ObserverFixture
    {
        ...
        {
            var root = Path.Combine(Path.GetTempPath(), "wevito-proposal-quality-observer-tests", Guid.NewGuid().ToString("N"));
        ...
        public ProposalQualityMetricsService CreateService(bool withWatchdog)
        {
            var watchdog = withWatchdog
                ? new InvariantViolationWatchdog(DatabasePath, Ledger, KillSwitch, () => Settings)
                : null;

            return new ProposalQualityMetricsService(DatabasePath, ArtifactRoot, KillSwitch, watchdog, () => Settings);
        }
